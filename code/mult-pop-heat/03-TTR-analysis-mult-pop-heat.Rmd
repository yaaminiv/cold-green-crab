...
I only have one timepoint for the NH experiment (no 22 hour heat stress), so I didn't include NH in the time analysis above. I will examine genotype's impact on TTR in these crabs.

```{r}
TTRmodelNH <- lmer(log(TTRavg) ~ as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                            (1|crab.ID), 
...
anova(TTRmodelNH, TTRfullGenotypeNH) #Significant impact of genotype
```
...
```{r}
var.lmerNH <- c(0.0000, 0.6723) #Save variances of random effects as a new vector (crab ID, residuals)
percentvar.lmerNH <- (100*var.lmerNH)/sum(var.lmerNH) #Calculate percent variances
percentvar.lmerNH[1] #Crab ID accounts for 6.416225% of variance
```
...
TTRModelComparisons <- rbind(broom::tidy(anova(TTRnullTimepointWA, TTRmodelWA)),
                             ...
                             broom::tidy(anova(TTRmodelNH, TTRfullGenotypeNH))) %>%
  mutate(., p.adj = p.adjust(p.value, method = "bonferroni")) #Create table for model comparison output. Adjust for multiple comparisons.
TTRModelComparisons #Confirm modifications
```
