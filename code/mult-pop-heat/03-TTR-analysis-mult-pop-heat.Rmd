...
rawTTRNH <- read.csv("../../../data/mult-pop-heat/NH_HSP_TTR.csv", header = TRUE, na.strings = c("N/A", "NA")) %>%
  dplyr::select(-carapace.length) #Import raw data and remove carapace length column
head(rawTTRNH)
```
...
```{r}
var.lmerMERevised <- c(0.06308, 0.16365) #Save variances of random effects as a new vector (crab ID, residuals)
percentvar.lmerMERevised <- (100*var.lmerMERevised)/sum(var.lmerMERevised) #Calculate percent variances
percentvar.lmerMERevised[1] #Crab ID accounts for 27.82164% of variance
```
...
There was no significant impact of timepoint on TTR.

```{r}
TTRmodelNHRevised <- lmer(log(TTRavg) ~ as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                            (1|crab.ID), 
                          data = modTTRNH, REML = FALSE) #Run model with timepoint, integument color, carapace width, weight, and missing swimmer information as fixed effects, and individual as a random effect. Also include parasite intensity. Change the likelihood estimator (REML = FALSE) to compare models with LRT later.
summary(TTRmodelNHRevised) #Look at model output.
#Residuals explain more random effect variation than crab ID, crab ID explains very little variation
```

```{r}
var.lmerNHRevised <- c(0.06574, 0.95885) #Save variances of random effects as a new vector (crab ID, residuals)
percentvar.lmerNHRevised <- (100*var.lmerNHRevised)/sum(var.lmerNHRevised) #Calculate percent variances
percentvar.lmerNHRevised[1] #Crab ID accounts for 6.416225% of variance
```

Same thing as the ME population: I only have CC and CT in this population, so I will only test for significance of genotype.

```{r}
TTRfullGenotypeNH <- lmer(log(TTRavg) ~ as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                            as.factor(genotype) + 
                            (1|crab.ID), 
                          data = modTTRNH, REML = FALSE) #Run model with genotype to identify significance of predictor
anova(TTRmodelNHRevised, TTRfullGenotypeNH) #Significant impact of genotype
```
...
TTRModelComparisons <- rbind(broom::tidy(anova(TTRnullTimepointWA, TTRmodelWA)),
                             ...
                             broom::tidy(anova(TTRnullTimepointNH, TTRmodelNH)),
                             # broom::tidy(anova(TTRnullIntensityNH, TTRmodelNH)),
                             broom::tidy(anova(TTRmodelNHRevised, TTRfullGenotypeNH))) %>%
  mutate(., p.adj = p.adjust(p.value, method = "bonferroni")) #Create table for model comparison output. Adjust for multiple comparisons.
TTRModelComparisons #Confirm modifications
```
...
rbind(modTTRWA %>%
  ...
  scale_x_discrete(name = "Exposure Time (Hours)",
                   labels = c(0, 22)) +
  scale_color_manual(values = c(plotColors[1], plotColors[2], plotColors[3]),
                     guide = "none") +
...
rbind(modTTRME %>%
  ...
  geom_boxplot(outlier.shape = NA) + 
  geom_jitter(position = position_dodge(width = 0.75)) +
  facet_grid(population~genotype, scale = "free_y") +
  scale_y_continuous(name = "Average Time-to-Right (s)") +
  scale_x_discrete(name = "Exposure Time (Hours)",
                   labels = c(0, 22)) +
  scale_color_manual(values = c(plotColors[1], plotColors[2], plotColors[3]),
...
ggsave("figures/time-to-right-multipanel.pdf", height = 11, width = 8.5)
```
