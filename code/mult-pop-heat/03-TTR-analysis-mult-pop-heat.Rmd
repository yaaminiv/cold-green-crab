...
```{r}
summary(TTRfullGenotypeME) #The final model has timepoint but not genotype
```

```{r}
var.lmerME <- c(0.04533, 0.16365) #Save variances of random effects as a new vector (crab ID, residuals)
percentvar.lmerME <- (100*var.lmerME)/sum(var.lmerME) #Calculate percent variances
percentvar.lmerME[1] #Crab ID accounts for 21.69107% of variance
```
...
TTRModelComparisonsME <- rbind(broom::tidy(anova(TTRnullSexME, TTRmodelMEDV)),
                               ...
                               broom::tidy(anova(TTRnullMetaME, TTRmodelMEDV)),
...
Since I'm using a GLM, I can put everything into the model and use stepwise backwards deletion to retain only significant variables.

### Assess importance of parasites

```{r}
TTRparasiteNHDV <- glm(log(TTRavg) ~ meta_Intensity + acanth_Intensity, 
                     data = modTTRNH) #Create model with all potential explanatory variables
summary(TTRparasiteNHDV) #No demographic variable is significant
```

### Assess importance of genotype

```{r}
TTRfullGenotypeNH <- glm(log(TTRavg) ~ as.factor(genotype),
                          data = modTTRNH) #Run model with just genotype
summary(TTRfullGenotypeNH) #Genotype significantly impacts TTR
```

### Save model output

```{r}
TTRmodelNHResults <- rbind(broom::tidy(TTRparasiteNHDV),
      broom::tidy(TTRfullGenotypeNH)) %>%
    mutate(., p.adj = p.adjust(p.value, method = "bonferroni")) #Create table for model comparison output. Adjust for multiple comparisons.
TTRmodelNHResults
```

```{r}
write.csv(TTRmodelNHResults, "ttr-model-comparison-stat-output-NH.csv", quote = FALSE, row.names = FALSE) #Save statistical output
```
...
TTRmodelNH_genotype$contrasts #t = -3.651, p = 0.0029
```
...
rbind(broom::tidy(TTRmodelWA_timepoint$contrasts) %>%
      ...
      broom::tidy(TTRmodelME_genotype$contrasts) %>%
        mutate(., population = "ME"),
      broom::tidy(TTRmodelNH_genotype$contrasts) %>%
