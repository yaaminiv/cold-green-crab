...
```{r}
rawTTRWA <- read.csv("../../../data/mult-pop-heat/WA_HSP_TTR.csv", header = TRUE, na.strings = c("N/A", "NA")) %>%
  dplyr::select(-genotype) #Import raw data
head(rawTTRWA)
```
...
```{r}
rawTTRME <- read.csv("../../../data/mult-pop-heat/ME_HSP_TTR.csv", header = TRUE, na.strings = c("N/A", "NA")) %>%
  dplyr::select(-genotype) #Import raw data
head(rawTTRME)
```
...
parasiteME <- read.csv("../../../data/mult-pop-heat/ME_HSP_Para.csv", header = TRUE) %>%
  dplyr::select(., -c(Species:CW, X)) #Import parasite data and select columns of interest
...
```{r}
treatment_labels <- c("1.5C" = "2.1ºC",
                      "5C" = "5.2ºC") #Modify facet labels for treatment
