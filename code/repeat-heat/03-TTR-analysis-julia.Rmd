...
```{r}
TTRfullGenotype <- lmer(log(TTRavg) ~ as.factor(treatment) + day + as.factor(treatment):day + as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + as.factor(genotype) + (1|crab.ID), data = modTTR, REML = FALSE) #Run model with genotype to identify4 significance of predictor
...
plotWithInset <- ggdraw() +
  draw_plot(mainPlot) +
  draw_plot(insetPlot, x = 0.1, y = 0.73, width = 0.52, height = 0.25) #Add inset to the plot
...
plotWithInsetMultipanel <- ggdraw() +
  ...
  draw_plot(insetPlotMultipanel, x = 0.1, y = 0.73, width = 0.52, height = 0.25) #Add inset to the plot
