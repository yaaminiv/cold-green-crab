...
```{r}
TTRmodel <- lmer(log(TTRavg) ~ as.factor(treatment) + as.factor(day) + as.factor(treatment):as.factor(day) + 
...
```{r}
var.lmer <- c(0.02907, 0, 0.20532) #Save variances of random effects as a new vector (crab ID, tank, residuals)
percentvar.lmer <- (100*var.lmer)/sum(var.lmer) #Calculate percent variances
percentvar.lmer[1] #Crab ID accounts for 12.40241% of variance
...
```{r}
TTRnullTreatment <- lmer(log(TTRavg) ~ as.factor(day) + as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + (1|crab.ID) + (1|tank), data = modTTR, REML = FALSE) #Run null model without treatment to identify significance of predictor
...
```{r}
TTRnullInteraction <- lmer(log(TTRavg) ~ as.factor(treatment) + as.factor(day) + as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + (1|crab.ID) + (1|tank), data = modTTR, REML = FALSE) #Run model without interaction to identify significance of predictor. The full model constructed does not include an interaction
anova(TTRnullInteraction, TTRmodel) #Interaction is a significant predictor
```

```{r}
TTRmodel_treatment <- lsmeans(TTRmodel, 
                              list(pairwise ~ as.factor(treatment)|as.factor(day), 
                                   pairwise ~ as.factor(day)|as.factor(treatment)),
                              adjust = "tukey") #Pairwise tukey test for timepoint and day accounting for the interaction
```

```{r}
TTRmodel_treatment$`pairwise differences of treatment | day` %>%
  broom::tidy(.) %>%
  dplyr::rename(treatment = "2") %>%
  write.csv("tukey-test-output-treatment.csv", quote = FALSE, row.names = FALSE)
#Significant effects on day 5 between control and treatment
```

```{r}
TTRmodel_treatment$`pairwise differences of day | treatment` %>%
  broom::tidy(.) %>%
  dplyr::rename(day = "2") %>%
  write.csv("tukey-test-output-day.csv", quote = FALSE, row.names = FALSE)
#Significant effects for no pulse between day 1 and 5 and day 2 and 5
```
...
TTRfullGenotype <- lmer(log(TTRavg) ~ as.factor(treatment) + as.factor(day) + as.factor(treatment):as.factor(day) + 
                          as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                          as.factor(genotype) + 
                          (1|crab.ID) + (1|tank), data = modTTR, REML = FALSE) #Run model with genotype to identify4 significance of predictor
...
TTRpresenceC <- lmer(log(TTRavg) ~ as.factor(treatment) + as.factor(day) + as.factor(treatment):as.factor(day) +
                       as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                       as.factor(presenceC) + 
                       (1|crab.ID) + (1|tank), data = modTTR, REML = FALSE) #Run model with genotype to identify significance of predictor
...
```{r}
mainPlot.stat.test <- rbind(TTRmodel_treatment$`pairwise differences of treatment | day` %>%
  broom::tidy(.) %>%
  dplyr::rename(constant = "day") %>%
  dplyr::rename(contrast = "2"),
  TTRmodel_treatment$`pairwise differences of day | treatment` %>%
  broom::tidy(.) %>%
  dplyr::rename(constant = "treatment") %>%
  dplyr::rename(contrast = "2") %>%
  dplyr::rename(p.value = "adj.p.value")) %>%
  filter(., p.value < 0.05) %>%
  mutate(., p.sig = c("**", "**", "****")) %>%
  mutate(., group1 = c("No Pulse", "day1", "day2"),
         group2 = c("Pulse", "day5", "day5")) %>%
  dplyr::select(., constant, term, group1, group2, statistic, p.value, p.sig)
  #Combine significant effects and use similar column names. Add a column for asterisk notation of significant effects. Rename contrast column for group 1 and group 2
mainPlot.stat.test
```
...
mainPlotMultipanel <- modTTR %>%
  ...
  annotate("rect", 
           ...
           ymin = 0, ymax = 15, 
           alpha = 0.2, fill = plotColors[1]) +
  geom_segment(aes(x = 0.82, xend = 2.8,
                   y = 15.3, yend = 15.3),
               color = "black", linewidth = 0.2) + #Add significance bar for day 1 vs. day 5
  annotate("text", x = (0.82+2.8)/2, y = 15.5, label = "**") + #Add significance asterisks for day 1 vs. day 5
  geom_segment(aes(x = 1.82, xend = 2.8,
                   y = 16, yend = 16),
               color = "black", linewidth = 0.2) + #Add significance bar for day 2 vs. day 5
  annotate("text", x = (1.82+2.8)/2, y = 16.2, label = "****") + #Add significance asterisks for day 2 vs. day 5
  geom_segment(aes(x = 2.8, xend = 3.2,
                   y = 16.7, yend = 16.7),
               color = "black", linewidth = 0.2) + #Add significance bar for treatment
  annotate("text", x = 3, y = 16.9, label = "**") + #Add significance asterisks for treatment
  scale_x_discrete(name = "",
                   labels = c("0", "1", "4")) +
  scale_y_continuous(name = "Average Time-to-Right (s)",
                     breaks = c(0:5, 10, 15),
...
plotWithInsetMultipanel <- ggdraw() +
  ...
  draw_plot(insetPlotMultipanel, x = 0.084, y = 0.37, width = 0.4, height = 0.25) #Add inset to the plot
