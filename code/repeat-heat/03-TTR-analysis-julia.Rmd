...
```{r}
TTRmodel_treatment <- emmeans(TTRmodel, pairwise ~ treatment | day, at = list(day = c(1, 2, 5)), adjust = "tukey", data = modTTR) #Use emmeans to obtain pairwise treatment differences at the timepoints listed. 
...
```{r}
TTRmodel_treatment_slope <- emtrends(TTRmodel, pairwise ~ treatment, var = "day", data = modTTR) #Obtain simple slopes for the interaction between each temperature and time. Test if the effect of time significantly differs between the two treatments
...
TTRmodel_factor <- lmer(log(TTRavg) ~ treatment + day_factor + treatment:day_factor + 
                          as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                          (1|crab.ID), 
                        data = (modTTR %>% filter(., is.na(TTRavg) == FALSE)), REML = FALSE) #Run the original model, but use timepoint as a factor
TTRmodel_day <- emmeans(TTRmodel_factor, pairwise ~ day_factor | treatment, adjust = "tukey", data = modTTR) #Rerun emmeans using timepoint as a factor
