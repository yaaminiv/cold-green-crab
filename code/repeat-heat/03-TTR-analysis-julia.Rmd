...
```{r}
TTRfullGenotype <- lmer(log(TTRavg) ~ as.factor(treatment) + day + as.factor(treatment):day + 
                          as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                          as.factor(genotype) + 
...
```{r}
TTRpresenceC <- lmer(log(TTRavg) ~ as.factor(treatment) + day + as.factor(treatment):day +
                       as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                       as.factor(presenceC) + 
...
```{r}
TTRpresenceT <- lmer(log(TTRavg) ~ as.factor(treatment) + day + as.factor(treatment):day + 
                       as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                       as.factor(presenceT) + 
                       (1|crab.ID) + (1|tank), data = modTTR, REML = FALSE) #Run model with genotype to identify significance of predictor
...
mainPlotMultipanel <- modTTR %>%
  ...
  geom_text(data = day_letters_plot,
            aes(x = day, y = label_y, label = .group, group = treatment, color = treatment),
...
plotWithInsetMultipanel <- ggdraw() +
  ...
  draw_plot(insetPlotMultipanel, x = 0.084, y = 0.62, width = 0.4, height = 0.25) #Add inset to the plot
