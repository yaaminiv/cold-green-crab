...
# Compare blank-corrected oxygen consumption across treatment
...
```{r}
lm20230804 <- lm((slope_nmol_hr_corrected * -1) ~ as.factor(treatment),
...
```{r}
hist(residuals(lm20230804)) #Residuals normally distributed

plot(fitted(lm20230804), residuals(lm20230804)) #Lack of dispersion across x-axis is likely related to discrete variables
...
```{r}
dat20230804_MO2_all_slope_results %>%
  ggplot(., aes(x = treatment, y = -slope_nmol_hr_corrected, color = treatment, shape = treatment)) + 
