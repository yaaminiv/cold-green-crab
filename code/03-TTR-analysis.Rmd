...
```{r}
TTRmodelSig <- glm(log(TTRavg) ~ treatment + timepoint + integument.cont + weight + treatment:timepoint, data = modTTR) #Run the most significant model identified by step, which includes treatment, timepoint, integument color, weight, and the interaction between treatment and timepoint
summary(TTRmodelSig) #Significant impact of treatment, timepoint, their interaction, integument color, and weight
```

```{r}
summary(TTRmodelSig) #Most parsimonious model. Significant influence of treatment, integment color, and weight, but no effect of timepoint or size.
```
