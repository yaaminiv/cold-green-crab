...
```{r}
var.lmer <- c(0.04731, 0.39013) #Save variances of random effects as a new vector (crab ID, residual)
percentvar.lmer <- (100*var.lmer)/sum(var.lmer) #Calculate percent variances
percentvar.lmer[1] #Crab ID accounts for 10.8152% of variance
```
...
pairs(TTRmodel_time) %>% broom::tidy(.) #Use the pairs function to look at pairwise output. Clean output using broom.
pairs(TTRmodel_time) %>% 
  broom::tidy(.) %>%
  write.csv("tukey-test-time.csv", quote = FALSE, row.names = FALSE) #Save output as a csv.
```
