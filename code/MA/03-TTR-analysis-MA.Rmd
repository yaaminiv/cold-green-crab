...
```{r}
TTRmodel <- lmer(log(TTRavg) ~ treatment + as.factor(timepoint) + treatment:as.factor(timepoint) + 
...
```{r}
var.lmer <- c(0.06529, 0.25870) #Save variances of random effects as a new vector (crab ID, residual)
percentvar.lmer <- (100*var.lmer)/sum(var.lmer) #Calculate percent variances
percentvar.lmer[1] #Crab ID accounts for 20.15186% of variance
```
...
```{r}
TTRnullTreatment <- lmer(log(TTRavg) ~ as.factor(timepoint) + as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + (1|crab.ID), data = (modTTR %>% filter(., is.na(TTRavg) == FALSE)), REML = FALSE) #Run null model without treatment to identify significance of predictor
...
anova(TTRnullTimepoint, TTRmodel) #Timepoint is a significant predictor
```

```{r}
TTRnullInteraction <- lmer(log(TTRavg) ~ as.character(treatment) + as.factor(timepoint) + as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + (1|crab.ID), data = (modTTR %>% filter(., is.na(TTRavg) == FALSE)), REML = FALSE) #Run model without interaction to identify significance of predictor. The full model constructed does not include an interaction
anova(TTRnullInteraction, TTRmodel) #Timepoint is a significant predictor
```

```{r}
TTRmodel_treatment <- emmeans(TTRmodel, pairwise ~ treatment | timepoint, at = list(time = c(0:5)), data = modTTR) #Use emmeans to obtain pairwise treatment differences at the timepoints listed. 
pairs(TTRmodel_treatment) %>% broom::tidy(.) #Use the pairs function to look at pairwise test output. Clean output using broom.
pairs(TTRmodel_treatment) %>% 
  broom::tidy(.) %>%
...
```{r}
TTRmodel_treatment <- lsmeans(TTRmodel, 
                              list(pairwise ~ treatment|as.factor(timepoint), 
                                   pairwise ~ as.factor(timepoint)|treatment),
                              adjust = "tukey",
                              data = modTTR) #Pairwise tukey test for timepoint and treatment accounting for the interaction
```

```{r}
TTRmodel_treatment$`pairwise differences of treatment | timepoint` %>%
  broom::tidy(.) %>%
  dplyr::rename(treatment = "2") %>%
  write.csv("tukey-test-treatment.csv")
#Significant differences between 1.5 and 5 at all timepoints except for hour 0
```

```{r}
TTRmodel_treatment$`pairwise differences of timepoint | treatment` %>%
  broom::tidy(.) %>%
  dplyr::rename(day = "2") %>%
  write.csv("tukey-test-timepoint.csv")
#Lots of significant pairwise differences
```
...
```{r}
TTRfullGenotype <- lmer(log(TTRavg) ~ treatment + as.factor(timepoint) + treatment:as.factor(timepoint) + as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + as.factor(genotype) + (1|crab.ID), data = (modTTR %>% filter(., is.na(TTRavg) == FALSE)), REML = FALSE) #Run model with genotype to identify significance of predictor
...
```{r}
TTRsource <- lmer(log(TTRavg) ~ treatment + as.factor(timepoint) + treatment:as.factor(timepoint) + as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + as.factor(source) + (1|crab.ID), data = (modTTR %>% filter(., is.na(TTRavg) == FALSE)), REML = FALSE) #Run model with genotype to identify significance of predictor
...
TTRModelComparisons <- TTRModelComparisons %>%
  mutate(p.adj = p.adjust(p.value, method = "bonferroni")) #Adjust p-values for multiple comparisons
```
...
```{r}
TTRthreshModel <- glmer(I(TTRthresh == 1) ~ treatment + as.factor(timepoint) + treatment:as.factor(timepoint) +
                          as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                          (1|crab.ID), 
                        family = binomial(), 
...
```{r}
TTRthreshNullTreatment <- glmer(I(TTRthresh == 1) ~ as.factor(timepoint) +
                                  as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                                  (1|crab.ID), 
                                family = binomial(), 
...
```{r}
TTRthreshNullInteraction <- glmer(I(TTRthresh == 1) ~ treatment + as.factor(timepoint) +
                                    as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                                    (1|crab.ID), 
                                  family = binomial(), 
...
```{r}
TTRthreshModelRevised <- glmer(I(TTRthresh == 1) ~ treatment + as.factor(timepoint) +
                                 as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                                 (1|crab.ID), 
                               family = binomial(), 
                               data = modTTR) #Create a binomial model, where 1 = success of righting wihtin the defined threshold
...
```{r}
TTRthreshNullSex <- glmer(I(TTRthresh == 1) ~ treatment + as.factor(timepoint) +
...
```{r}
TTRthreshGenotype <- glmer(I(TTRthresh == 1) ~ treatment + as.factor(timepoint) +
                             as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                             as.factor(genotype) +
                             (1|crab.ID), 
                           family = binomial(), 
...
```{r}
TTRthreshpresenceC <- glmer(I(TTRthresh == 1) ~ treatment + as.factor(timepoint) +
                              as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                              as.factor(presenceC) +
                              (1|crab.ID), 
                            family = binomial(), 
...
```{r}
TTRthreshpresenceT <- glmer(I(TTRthresh == 1) ~ treatment + as.factor(timepoint) +
                              as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                              as.factor(presenceT) +
                              (1|crab.ID), 
                            family = binomial(), 
...
```{r}
TTRthreshSource <- glmer(I(TTRthresh == 1) ~ treatment + as.factor(timepoint) +
                           as.factor(sex) + integument.cont + carapace.width + weight + as.factor(missing.swimmer) + 
                           as.factor(source) +
                           (1|crab.ID), 
                         family = binomial(), 
...
```{r}
treatment_labels <- c("0C" = "1.7ºC",
                      "5C" = "5.4ºC") #Modify facet labels for treatment
