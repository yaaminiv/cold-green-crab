...
```{r}
var.lmer <- c(0.02667, 0.47509) #Save variances of random effects as a new vector (crab ID, residual)
percentvar.lmer <- (100*var.lmer)/sum(var.lmer) #Calculate percent variances
percentvar.lmer[1] #Crab ID accounts for 5.31529% of variance
```
...
```{r}
write.csv(TTRModelComparisons, "ttr-model-comparison-stat-output-WA.csv", quote = FALSE, row.names = FALSE) #Save model output
```
...
```{r}
TTRmodel_treatment <- emmeans(TTRmodelEV, pairwise ~ treatment | timepoint, at = list(timepoint = c(0, 1, 2, 3, 4, 5)), adjust = "tukey", data = modTTR) #Use emmeans to obtain pairwise treatment differences at the timepoints listed. 
TTRmodel_treatment$contrasts %>% broom::tidy(.) #Use the pairs function to look at pairwise test output. Clean output using broom.
...
```{r}
TTRmodel_treatment_slope <- emtrends(TTRmodelEV, pairwise ~ treatment, var = "timepoint", data = modTTR) #Obtain simple slopes for the interaction between each temperature and time. Test if the effect of time significantly differs between the two treatments
...
TTRmodel_time$contrasts %>% broom::tidy(.) #Use the pairs function to look at pairwise output. Clean output using broom.
...
modTTRSubset <- modTTR %>%
  ...
  dplyr::select(., -c(date:tank, TTRavg.y:TTRSE)) %>%
...
Will not be testing missing swimmer since no crabs were missing a swimmer leg.

```{r}
TTRSubsetDV <- lmer(log(TTRavg) ~ as.factor(sex) + integument.cont + carapace.width + weight + 
                     (1|crab.ID), 
...
```{r}
TTRSubsetSex <- lmer(log(TTRavg) ~ integument.cont + carapace.width + weight + 
                     (1|crab.ID), 
...
```{r}
TTRSubsetIntegument <- lmer(log(TTRavg) ~ as.factor(sex) + carapace.width + weight + 
                            (1|crab.ID), 
                          data = modTTRSubset, REML = FALSE) #Run model without variable of interest to identify significance
...
```{r}
TTRSubsetCW <- lmer(log(TTRavg) ~ as.factor(sex) + integument.cont + weight +
                    (1|crab.ID), 
                  data = modTTRSubset, REML = FALSE) #Run model without variable of interest to identify significance
...
```{r}
TTRSubsetWeight <- lmer(log(TTRavg) ~ as.factor(sex) + integument.cont + carapace.width + 
                        (1|crab.ID), 
                      data = modTTRSubset, REML = FALSE) #Run model without variable of interest to identify significance
...
I could only test the full genotype, since there were only CT and TT crabs left at the end.
...
```{r}
var.lmer <- c(0.002613, 0.538479) #Save variances of random effects as a new vector (crab ID, residual)
percentvar.lmer <- (100*var.lmer)/sum(var.lmer) #Calculate percent variances
percentvar.lmer[1] #Crab ID accounts for 0.4829123% of variance in the subset model
```
...
```{r}
write.csv(TTRSubsetModelComparisons, "ttr-subset-model-comparison-stat-output-WA.csv", quote = FALSE, row.names = FALSE) #Save model output
```
...
```{r}
write.csv(TTRthreshModelComparisons, "ttr-binomial-model-comparison-output-WA.csv", quote = FALSE, row.names = FALSE) #Save model comparison table
```
...
```{r}
TTRthreshSubsetModelDV <- glmer(I(TTRthresh == 1) ~ as.factor(sex) + integument.cont + carapace.width + weight + 
                            (1|crab.ID), 
                          family = binomial(), 
...
```{r}
TTRthreshSubsetSex <- glmer(I(TTRthresh == 1) ~ integument.cont + carapace.width + weight +
                        (1|crab.ID), 
                      family = binomial(), 
                      data = modTTRSubset)
...
```{r}
TTRthreshSubsetIntegument <- glmer(I(TTRthresh == 1) ~ as.factor(sex) + carapace.width + weight +
                               (1|crab.ID), 
                             family = binomial(), 
...
```{r}
TTRthreshSubsetCW <- glmer(I(TTRthresh == 1) ~ as.factor(sex) + integument.cont + weight +
                       (1|crab.ID), 
                     family = binomial(), 
                     data = modTTRSubset) #Create a binomial model, where 1 = success of righting wihtin the defined threshold
...
```{r}
TTRthreshSubsetModelWeight <- glmer(I(TTRthresh == 1) ~ as.factor(sex) + integument.cont + carapace.width +
                                (1|crab.ID), 
                              family = binomial(), 
                              data = modTTRSubset) #Create a binomial model, where 1 = success of righting wihtin the defined threshold
...
Unlike the full model, time is not a significant factor.
...
```{r}
write.csv(TTRthreshSubsetComparisons, "ttr-binomial-subset-model-comparison-output-WA.csv", quote = FALSE, row.names = FALSE) #Save model comparison table
```

## Revised final model

Since time was not significant in the sensitivity test, I will remove it from the final model.

```{r}
TTRthreshModelRevised2 <- glmer(I(TTRthresh == 1) ~ treatment +
                                 (1|crab.ID), 
                               family = binomial(), 
                               data = modTTR) #Create a binomial model, where 1 = success of righting wihtin the defined threshold
```
...
```{r}
TTRthreshModelRevisedNull <- glmer(I(TTRthresh == 1) ~ (1|crab.ID), 
                               family = binomial(), 
                               data = modTTR) #Create a binomial model, where 1 = success of righting wihtin the defined threshold
anova(TTRthreshModelRevisedNull, TTRthreshModelRevised2) #Temperature still significant
```

## Save revised model output
...
TTRthreshModelComparisons <- rbind(broom::tidy(anova(TTRthreshSex, TTRthreshModelDV)),
                                   ...
                                   broom::tidy(anova(TTRthreshModelRevised, TTRthreshpresenceC)),
                                   broom::tidy(anova(TTRthreshModelRevised, TTRthreshpresenceT)),
                                   broom::tidy(anova(TTRthreshModelRevisedNull, TTRthreshModelRevised2))) %>%
  mutate(p.adj = p.adjust(p.value, method = "bonferroni")) #Create a table of model comparison output. Adjust p-values.
TTRthreshModelComparisons
```
...
averageTTRWA <- modTTR %>%
  ...
  scale_shape_manual(values = c(15, 19),
                     ...
                     breaks = c("1.5C", "5C"),
                     labels = c("1.5ºC", "5ºC")) +
  ggtitle("A. Righting Response for WA Crabs") +
...
```{r}
averageTTRWAGenotype <- modTTR %>%
  dplyr::select(., c(timepoint, treatment, presenceT, TTRavg)) %>%
  filter(., is.na(TTRavg) == FALSE) %>%
  distinct(.) %>%
  mutate(., presenceTtreat = case_when(presenceT == "Y" & treatment == "1.5C" ~ "1_Y_0C",
                                       presenceT == "N" & treatment == "1.5C" ~ "2_N_0C",
...
```{r}
averageTTRWA / averageTTRWAGenotype / (failureToRightWA | genotypeFailureWA  + plot_layout(guides = "collect"))
ggsave("figures/multipanel-WA-plot.pdf", width = 8.5, height = 11)
```
...
```{r}
modTTRQ10 <- modTTR %>%
  dplyr::select(., timepoint, crab.ID, TTRavg, treatment, genotype) %>%
  group_by(., crab.ID) %>%
  pivot_wider(., names_from = timepoint, values_from = TTRavg, names_prefix = "timepoint") %>%
  mutate_all(~ifelse(is.nan(.), 91, .)) %>%
  filter(., is.na(timepoint0) == FALSE) %>%
  ...
  mutate(., timepoint5_Q10 = Q10(R2 = timepoint0, 
                                 ...
                                 T2 = 15, 
                                 T1 = temperature)) %>%
  dplyr::select(., crab.ID, treatment, genotype,
...
left_join(modTTRQ10, modTTR, by = c("crab.ID", "treatment", "genotype", "timepoint")) %>%
  dplyr::select(., -c(Q10:trial.3, sex:integument.cont, TTRSE)) %>%
...
ggsave("figures/time-to-right-individuals-5C-WA.pdf", height = 8.5, width = 11)
```

```{r}
left_join(modTTRQ10, modTTR, by = c("crab.ID", "treatment", "genotype", "timepoint")) %>%
  dplyr::select(., -c(date:trial.3, sex:presenceT)) %>%
  filter(., treatment != "5C") %>%
...
```{r}
all1.5CWA <- left_join(modTTRQ10, modTTR, by = c("crab.ID", "treatment", "genotype", "timepoint")) %>%
  dplyr::select(., -c(date:trial.3, sex:presenceT)) %>%
  filter(., treatment != "5C") %>%
...
```{r}
all5CWA <- left_join(modTTRQ10, modTTR, by = c("crab.ID", "treatment", "genotype", "timepoint")) %>%
  dplyr::select(., -c(Q10:trial.3, sex:integument.cont, TTRSE)) %>%
  filter(., treatment != "1.5C") %>%
  ggplot(., mapping = aes(x = timepoint, y = TTRavg, group = crab.ID)) +
  geom_point(size = 2, color = plotColors[2]) + geom_line(lty = 3, color = plotColors[2]) +
  ...
  scale_x_continuous(name = "Exposure Time (Hours)",
                     ...
                     labels = c(4, "", 28, "", 94)) +
  scale_y_continuous(name = "") +
  ggtitle("D. WA Crabs at 5ºC") +
  theme_classic(base_size = 12) + theme(strip.background = element_rect(colour = "white")) #Join TTR data with Q10 data to get individuals that were tracked throughout time. Remove superfluous columns and entries at 5ºC. Create point and line graph and wrap by crab ID.
all5CWA
```

```{r}
all1.5CMA + all1.5CWA + all5CMA + all5CWA #Create supp figure
ggsave("figures/multipanel-supp-tracked-crabs.pdf", height = 8.5, width = 11)
```
