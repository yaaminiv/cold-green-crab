...
```{r}
temp <- as.data.table(tank3a)[as.data.table(tank3b), on = "dateTime", roll = T][as.data.table(tank4a), on = "dateTime", roll = T][as.data.table(tank4b), on = "dateTime", roll = T][as.data.table(tank6a), on = "dateTime", roll = T][as.data.table(tank6b), on = "dateTime", roll = T][as.data.table(tank7a), on = "dateTime", roll = T][as.data.table(tank7b), on = "dateTime", roll = T][as.data.table(tank8a), on = "dateTime", roll = T][as.data.table(tank1a), on = "dateTime", roll = T] #Use data.table rolling join to match HOBO logger data together by nearest dateTime. Use only the loggers wiht at least 411 observations
temp
```
...
temp %>%
  dplyr::select(dateTime, 
                ...
                temp8a) %>%
  rowwise() %>%
  mutate(., colderTemp = mean(c(temp1a,
                                temp3a, temp3b,
                                temp4a, temp4b)),
         coldTemp = mean(c(temp6a, temp6b,
                           temp7a, temp7b,
                           temp8a))) %>%
